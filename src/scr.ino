...
void mulai()
{
  ...
  while (millis() - start < 10000)
  {
    ...
    showperjamsholat(0, "Subuh", 42, 0, 42, 9);
    showperjamsholat(2, "Dzuhur", 84, 0, 84, 9);
    showperjamsholat(3, "Ashar", 126, 0, 126, 9);
    showperjamsholat(4, "Magrib", 39, 0, 39, 9);
    showperjamsholat(6, "Isya'", 84, 0, 84, 9);
    showperjamsholat(8, "Imsak'", 126, 0, 126, 9);

    //INTRUPSI MASUK MENU//
    masukmenu();
  }

  //teksberjalan(bool isjam, char *teks1, int x1, int y1, char *teks2)
  teksberjalan(true, "Masjid Al HADI DULIDO", 4, 0, "luruhashdajsdk jadashdk j");

  //cekjamsholat(int isjumat, int startfrom, int urutansholatke, char *teks1, int x1, int y1, char *teks2, int x2, int y2, int lamaazanhitungmunduriqomah, int lamaazankeiqomah, int delaywaktusholat);
  cekjamsholat(0, 80000, 0, "Waktunya Azan ", 10, 5, "Subuh", 110, 5, 150000, 10, 300);
  cekjamsholat(1, 85000, 2, "Waktunya Azan ", 10, 5, "Dzuhur", 110, 5, 150000, 10, 300);
  cekjamsholat(2, 85000, 2, "Waktunya Azan ", 10, 5, "Dzuhur", 110, 5, 150000, 10, 300);
  cekjamsholat(0, 90000, 3, "Waktunya Azan ", 10, 5, "Ashar", 110, 5, 150000, 10, 300);
  cekjamsholat(0, 95000, 4, "Waktunya Azan ", 10, 5, "Magrib", 110, 5, 150000, 10, 300);
  cekjamsholat(0, 100000, 6, "Waktunya Azan ", 10, 5, "Isya'", 110, 5, 150000, 10, 300);
}

void showperjamsholat(int urutansholatke, String Teks, int x, int y, int x1, int y1)
{
  ...
  Teks.toCharArray(tampil, pj);
  dmd.selectFont(SystemFont5x7);
  dmd.drawString(x1, y1, tampil, pj, 0); //koordinat tampilan
...
void cekjamsholat(int isjumat, int startfrom, int urutansholatke, char *teks1, int x1, int y1, char *teks2, int x2, int y2, int lamaazanhitungmunduriqomah, int lamaazankeiqomah, int delaywaktusholat)
{
  ...
  while (millis() - start < startfrom)
  {
...

    {
      ...
      dmd.clearScreen(true);
      dmd.selectFont(SystemFont5x7);
      dmd.drawString(x1, y1, teks1, 15, 0); //koordinat tampilan
      dmd.selectFont(SystemFont5x7);
      dmd.drawString(x2, y2, teks2, 5, 0); //koordinat tampilan
      delay(lamaazanhitungmunduriqomah);
      ...
      if (isjumat != 2)
      {
        ...
        int tampil;
        int detik = 60, menit = lamaazankeiqomah;
        for (detik = 60; detik >= 0; detik--)
        {
          ...
          if (menit <= 0 && detik <= 1)
          {
            ...
            for (tampil = 0; tampil < delaywaktusholat; tampil++)
            {
              menit = 0;
              detik = 0;
              dmd.clearScreen(true);
      ...
      else
      {
        ...
        for (tampil = 0; tampil < 1550; tampil++)
        {
          ...
          dmd.selectFont(SystemFont5x7);
          dmd.drawString(9, 0, "SAAT KHOTBAH, HARAP DIAM", 24, 0); //koordinat tampilan
          ///////////////////////
          now = rtc.now();
          String xjam = Konversi(now.hour()) + ":" + Konversi(now.minute()) + ":" + Konversi(now.second()); //tampilan jam
          int pjg = xjam.length() + 1;
...
void teksberjalan(bool isjam, char *teks1, int x1, int y1, char *teks2)
{
  dmd.clearScreen(true);
  while (millis() - start < 20000)
  {
    ...
    if (isjam = true)
    {
      String hariku = teks1 + Konversi(now.hour()) + ":" + Konversi(now.minute());
    }
    String hariku = teks1;
    int dowo = hariku.length() + 1;
    char detikanku[dowo];
